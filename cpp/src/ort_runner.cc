...
#include "qwen3asr/build_config.h"
...
std::vector<Ort::Value> OrtRunner::RunIo(std::unordered_map<std::string, Ort::Value>& inputs,
  ...
  for (const auto& name : input_names_) {
    ...
    binding.BindInput(name.c_str(), it->second);
  }
  for (const auto& name : outputs) {
    const bool keep_on_device = options_.use_cuda && device_outputs.find(name) != device_outputs.end();
    binding.BindOutput(name.c_str(), keep_on_device ? cuda_memory_ : cpu_memory_);
  }
