...
// 而是覆盖 Qwen3-ASR 推理文本里最关键的路径：special token、中文、
...
void Qwen2BpeTokenizer::LoadVocab(const std::filesystem::path& path) {
  ...
  for (auto it = std::sregex_iterator(text.begin(), text.end(), item); it != std::sregex_iterator(); ++it) {
    const auto token = JsonUnescape((*it)[1].str());
    const auto id = std::stoll((*it)[2].str());
    vocab_[token] = id;
...
std::string Qwen2BpeTokenizer::Decode(const std::vector<int64_t>& ids, bool skip_special_tokens) const {
  std::string out;
  for (int64_t id : ids) {
    auto added = id_to_added_token_.find(id);
    if (added != id_to_added_token_.end()) {
      if (!skip_special_tokens) out += added->second;
      continue;
    }
    auto token_it = id_to_token_.find(id);
    if (token_it == id_to_token_.end()) continue;
    const std::string& token = token_it->second;
    for (size_t i = 0; i < token.size();) {
      size_t best_len = 0;
      unsigned char byte = 0;
      for (const auto& item : byte_decoder_) {
        const auto& piece = item.first;
        if (piece.size() > best_len && i + piece.size() <= token.size() &&
            token.compare(i, piece.size(), piece) == 0) {
          best_len = piece.size();
          byte = item.second;
        }
      }
