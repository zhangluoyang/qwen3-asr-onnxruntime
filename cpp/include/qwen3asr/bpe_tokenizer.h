...
// Qwen3-ASR Base 的文本侧沿用 Qwen2 byte-level BPE tokenizer。
...
class Qwen2BpeTokenizer {
 public:
  ...
  std::string Decode(const std::vector<int64_t>& ids, bool skip_special_tokens = true) const;
 ...
 private:
  ...
  std::unordered_map<int64_t, std::string> id_to_added_token_;
